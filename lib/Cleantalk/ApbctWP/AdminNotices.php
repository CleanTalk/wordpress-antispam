...
class AdminNotices
{
    ...
    public function notice_trial() // phpcs:ignore PSR1.Methods.CamelCapsMethodName.NotCamelCaps
    {
        ...
        if ( $this->apbct->notice_show && $this->apbct->notice_trial == 1 && $this->apbct->moderate_ip == 0 && ! $this->apbct->white_label ) {
            $content            = sprintf(
                ...
                "<a href='{$this->settings_link}'>" . $this->apbct->plugin_name . "</a>",
                "<a href=\"https://p.cleantalk.org/?account=undefined&currency=USD&domains=&extra=true&featured=4&fua=true&period=Year&period_interval=3&product_id=1&renew=true&user_token=$this->user_token\" target=\"_blank\"><b>premium version</b></a>"
            );
            $additional_content =
                '<h4 style = "color: gray">' .
    ...
    public function notice_renew() // phpcs:ignore PSR1.Methods.CamelCapsMethodName.NotCamelCaps
    {
        ...
        if ( $this->apbct->notice_show && $this->apbct->notice_renew == 1 && $this->apbct->moderate_ip == 0 && ! $this->apbct->white_label ) {
            $renew_link  = "<a href=\"https://p.cleantalk.org/?account=undefined&currency=USD&domains=&extra=true&featured=4&fua=true&period=Year&period_interval=3&product_id=1&renew=true&user_token=$this->user_token\" target=\"_blank\">%s</a>";
            $button_html = sprintf(
                $renew_link,
