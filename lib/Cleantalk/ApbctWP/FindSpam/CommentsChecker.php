...
class CommentsChecker extends Checker
{
    public function __construct()
    {
        ...
        wp_localize_script('ct_comments_checkspam', 'ctCommentsCheck', array(
            ...
            'ct_status_string'         => __(
                "Checked %s (without admin's comments), found %s spam comments and %s non-checkable comments (without IP and email).",
                'cleantalk-spam-protect'
            ),
    ...
    public static function ctAjaxInfo($direct_call = false)
    {
        ...
        if ( ! $direct_call ) {
            ...
            if ( $res ) {
                $return['message'] .= sprintf(
                    __(
                        "Last check %s: checked %s (without admin's comments), found %s spam comments and %s non-checkable comments (without IP and email).",
                        'cleantalk-spam-protect'
                    ),
                    self::lastCheckDate(),
                    $cnt_checked,
