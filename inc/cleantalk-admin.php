...
function apbct_add_buttons_to_comments_and_users($_unused_argument)
{
    ...
    <a href="' . $button_url__check . '" class="button" style="margin:1px 0 0 0; display: inline-block;">
        <img src="' . esc_url($apbct->logo__small__colored) . '" alt="CleanTalk Anti-Spam logo"  height="" style="width: 17px; vertical-align: text-bottom;" />
...
function ct_dashboard_statistics_widget_output($_post, $_callback_args)
{
    ...
    <div class='ct_widget_top_links'>
        <img src="<?php echo esc_url(APBCT_IMG_ASSETS_PATH . '/preloader.gif'); ?>" class='ct_preloader'>
        <?php
        echo sprintf(
    ...
    if ( isset($current_user) && in_array('administrator', $current_user->roles) ) {
        if ( $apbct->spam_count && $apbct->spam_count > 0 ) {
            echo '<div class="ct_widget_wprapper_total_blocked">'
                 . '<img src="' . esc_url($apbct->logo__small__colored) . '" class="ct_widget_small_logo"/>'
                 . '<span title="' . sprintf(
                     __(
...
function apbct_admin__enqueue_scripts($hook)
{
    ...
    wp_localize_script('ct_admin_js_notices', 'ctAdminCommon', array(
        ...
        'logo'               => '<img src="' . esc_url($apbct->logo) . '" alt=""  height="" style="width: 17px; vertical-align: text-bottom;" />',
        'logo_small'         => '<img src="' . esc_url($apbct->logo__small) . '" alt=""  height="" style="width: 17px; vertical-align: text-bottom;" />',
        'logo_small_colored' => '<img src="' . esc_url($apbct->logo__small__colored) . '" alt=""  height="" style="width: 17px; vertical-align: text-bottom;" />',
    ));
...
{
    return $icon
           . '<img class="cleantalk_admin_bar__apbct_icon" src="' . esc_url(APBCT_URL_PATH . '/inc/images/logo.png') . '" alt="">&nbsp;';
}
