...
function apbct_settings__display()
{
    ...
    if (
        ...
        echo '<button type="button" class="cleantalk_link cleantalk_link-auto" id="apbct_button__sync" title="Synchronizing account status, SpamFireWall database, all kind of journals.">'
             ...
             . '<img style="margin-left: 10px;" class="apbct_preloader_button" src="' . esc_url(APBCT_URL_PATH . '/inc/images/preloader2.gif') . '" />'
             . '<img style="margin-left: 10px;" class="apbct_success --hide" src="' . esc_url(APBCT_URL_PATH . '/inc/images/yes.png') . '" />'
             . '</button>';
        echo '&nbsp;&nbsp;';
    }
...
function apbct_settings__field__state()
{
    ...
    echo '<img class="apbct_status_icon" src="' . ($apbct->settings['forms__registrations_test'] == 1 ? esc_url($img) : esc_url($img_no)) . '"/>' . __(
        'Registration forms',
        'cleantalk-spam-protect'
    );
    echo '<img class="apbct_status_icon" src="' . ($apbct->settings['forms__comments_test'] == 1 ? esc_url($img) : esc_url($img_no)) . '"/>' . __(
        'Comments forms',
        'cleantalk-spam-protect'
    );
    echo '<img class="apbct_status_icon" src="' . ($apbct->settings['forms__contact_forms_test'] == 1 ? esc_url($img) : esc_url($img_no)) . '"/>' . __(
        'Contact forms',
        'cleantalk-spam-protect'
    );
    echo '<img class="apbct_status_icon" src="' . ($apbct->settings['forms__general_contact_forms_test'] == 1 ? esc_url($img) : esc_url($img_no)) . '"/>' . __(
        'Custom contact forms',
        'cleantalk-spam-protect'
    );
    if ( ! $apbct->white_label || is_main_site() ) {
        echo '<img class="apbct_status_icon" src="' . ($apbct->data['moderate'] == 1 ? esc_url($img) : esc_url($img_no)) . '"/>'
    ...
    if ( $apbct->notice_auto_update && ( ! $apbct->white_label || is_main_site()) ) {
        echo '<img class="apbct_status_icon" src="' . ($apbct->auto_update == 1 ? esc_url($img) : ($apbct->auto_update == -1 ? esc_url($img_no) : esc_url($img_no_gray))) . '"/>' . __(
            'Auto update',
            'cleantalk-spam-protect'
        )
    ...
    if ( class_exists('WooCommerce') ) {
        echo '<img class="apbct_status_icon" src="' . ($apbct->settings['forms__wc_checkout_test'] == 1 ? esc_url($img) : esc_url($img_no)) . '"/>' . __(
